...
**② CLI 模式 —— 一行命令,自带 provider key。** 自动化、可脚本化,退出码直接进 CI。
...
`review` 的**退出码就是 Verdict**(`PROCEED=0 / HOLD=1 / ARCHIVE=2`)——直接塞进 CI。手头没 key?`falsify lint <file>` 跑纯本地 ship-blocker 检查(零 API):
